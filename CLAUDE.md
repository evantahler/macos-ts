...
Photos tests run against `tests/fixtures/Photos.sqlite` — a checked-in SQLite database with 6 visible assets (+ 1 trashed) and 3 albums. The fixture is generated by `tests/fixtures/create-photos-db.ts`. It covers photos and videos, favorites, hidden assets, iCloud-only assets, GPS coordinates, smart and user albums, and album membership.
