...
Photos tests run against `tests/fixtures/Photos.sqlite` — a checked-in SQLite database with 7 visible assets (+ 1 trashed) and 3 albums. The fixture is generated by `tests/fixtures/create-photos-db.ts`. It covers photos and videos, favorites, hidden assets, iCloud-only photos and videos, GPS coordinates, smart and user albums, and album membership. Video assets get both a master `ZINTERNALRESOURCE` row (`ZRESOURCETYPE=1`) and a poster row (`ZRESOURCETYPE=0`) with inverse availability so `CHECK_LOCAL_AVAILABILITY` is exercised correctly for videos.
